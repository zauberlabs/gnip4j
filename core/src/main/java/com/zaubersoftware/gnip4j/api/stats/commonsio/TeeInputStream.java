 ...
 */
package com.zaubersoftware.gnip4j.api.stats.commonsio;
 ...
 * Using {@link #skip(long)} or {@link #mark(int)}/{@link #reset()} on the stream will result on some bytes from the
 * input stream being skipped or duplicated in the output stream.
 * <p>
 * The proxied input stream is closed when the {@link #close()} method is called on this proxy. It is configurable
 * whether the associated output stream will also closed.
 * 
...
public class TeeInputStream extends ProxyInputStream {
    ...
    /**
     * Creates a TeeInputStream that proxies the given {@link InputStream} and copies all read bytes to the given
     * {@link OutputStream}. The given output stream will not be closed when this stream gets closed.
     * 
     * @param input
    ...
    /**
     * Creates a TeeInputStream that proxies the given {@link InputStream} and copies all read bytes to the given
     * {@link OutputStream}. The given output stream will be closed when this stream gets closed if the closeBranch
    ...
    /**
     * Closes the proxied input stream and, if so configured, the associated output stream. An exception thrown from one
     * stream will not prevent closing of the other stream.
     * 
     * @throws IOException
     ...
     *             if the stream could not be read (or written)
     */
    public final int read(final byte[] bts, final int st, final int end) throws IOException {
